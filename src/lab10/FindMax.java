...
public class FindMax {
	...
	public void findMaxByThread() {
        ExecutorService executor = Executors.newFixedThreadPool(3);

        executor.execute(new Task(0,29));
        executor.execute(new Task(30,59));
        executor.execute(new Task(60,89));
    ...
    private class Task implements Runnable {
        ...
        @Override
        public void run() {
            String range = "Among " + start + " to " + end + " ,";
            System.out.println(range + "the max value is " + new FindMax().findMax(start,end));
        }
    }
