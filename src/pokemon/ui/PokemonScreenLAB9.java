...
public class PokemonScreenLAB9 extends Application {
	...
	public void start(Stage stage) throws Exception {
        ...
        System.out.println("ox: " + currentPosx + ", oy: " + currentPosy + "\n");

        // add Pikachu
        pikachuImage = new Image(new File("icons/pikachu.gif").toURI().toString());
	...
	private void moveAvatarBy(int dx, int dy) {
        double ax = avatar.getLayoutX();
        double ay = avatar.getLayoutY();
	...
	private void moveAvatar(double x, double y) {
		...
		if (x - cx >= 0 && x + cx <= W && y - cy >= 0 && y + cy <= H) {
            // relocate ImageView pikachu before avatar
